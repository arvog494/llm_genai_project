...
try:
    ...
    GraphRAGService = None  # type: ignore[assignment]
    GRAPH_RAG_IMPORT_ERROR: Optional[Exception] = exc
else:
    GRAPH_RAG_IMPORT_ERROR = None
...
class GraphRAGEvalAdapter:
    """Adapter that exposes GraphRAGService behind the simple .query API used by evals."""

    def __init__(self, logdir: str = "logs"):
        if GraphRAGService is None:
            raise RuntimeError(
                "Unable to import GraphRAGService. Ensure the repository's src/ folder is accessible."
            ) from GRAPH_RAG_IMPORT_ERROR

        self._service = GraphRAGService()
        if not getattr(self._service, "index_built", False):
            raise RuntimeError(
                "GraphRAG index not found. Run `python src/main_build_index.py` before evaluating."
            )

        log_path = Path(logdir)
        if not log_path.is_absolute():
            log_path = PACKAGE_ROOT / log_path
        self.logdir = log_path
        self.logdir.mkdir(parents=True, exist_ok=True)

    def query(self, question: str, **_: Any) -> Dict[str, Any]:
        response = self._service.answer(question)
        log_file = self._write_log(question, response)
        return {
    ...
    def _write_log(self, question: str, result: Dict[str, Any]) -> Path:
        timestamp = datetime.utcnow().isoformat()
        filename = f"graph_rag_eval_{timestamp.replace(':', '-')}.json"
        log_path = self.logdir / filename
        payload = {
            "timestamp": timestamp,
            "question": question,
            "result": result,
...
def default_rag_client(llm_client, logdir: str = "logs") -> GraphRAGEvalAdapter:
    del llm_client  # not needed when delegating to GraphRAGService
    return GraphRAGEvalAdapter(logdir=logdir)
